 ...
 * @Author: Soulmate
 * @Date: 2023-02-22 11:12:54
 * @LastEditTime: 2023-03-05 16:40:04
 * @LastEditors: Soulmate
 * @Description: 
 * @FilePath: \template\src\views\system\menu\components\Perm.vue
 * 版权声明
...
function handleUpdate(row: any) {
  ...
  getPermFormDetail(id, props.menuId).then((response) => {
    const { data } = response;
    state.formData = data;
    console.log(data && data.urlPerm)
  ...
  dataFormRef.value.validate((isValid: any) => {
    if (isValid) {
      ...
      state.formData.urlPerm =
          ...
          state.formData.requestMethod = state.urlPerm.requestMethod
          state.formData.serviceName = state.urlPerm.serviceName
          state.formData.requestPath = state.urlPerm.requestPath
      state.formData.menuId = props.menuId;
      if (state.formData._id) {
        updatePerm(state.formData).then(() => {
          ElMessage.success('修改成功');
          cancel();
          handleQuery();
        });
      } else {
        addPerm({...state.formData, ...state.urlPerm}).then(() => {
...
function handleDelete(row: any) {
  ...
  ElMessageBox.confirm('确认删除已选中的数据项?', '警告', {
    ...
    .then(() => {
      deletePerms(ids, props.menuId).then(() => {
  ...
  <div claas="component-container">
    ...
    >
      ...
      <el-table-column label="URL权限" align="center">
        <el-table-column prop="serviceName" label="所属服务" align="center" />
        <el-table-column prop="requestMethod" label="请求方式" align="center" />
        <el-table-column prop="requestPath" label="请求路径" align="center" />
      </el-table-column>
      <el-table-column label="按钮权限" prop="btnPerm" width="200" align="center" />
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="700px">
      ...
      >
        ...
        <el-form-item label="是否写入日志" prop="isWriteLog">
          <el-radio-group v-model="formData.isWriteLog">
